...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> upKey = root.put(key, rid);
        if (!upKey.isPresent()) {
            return;
        }
        // split the root and upgrade to InnerNode
        List<DataBox> keys = new ArrayList<>();
        List<Long> children = new ArrayList<>();
        keys.add(upKey.get().getFirst());
        // root also as child now
        children.add(root.getPage().getPageNum());
        // new child
        children.add(upKey.get().getSecond());

        // upgrade to Innernode
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        if(scanAll().hasNext()){
            throw new BPlusTreeException("Empty tree is required for bulkLoad");
        }
        while(data.hasNext()){
            ...
            List<DataBox> keys = new ArrayList<>();
            List<Long> children = new ArrayList<>();
            keys.add(up.get().getFirst());
            children.add(root.getPage().getPageNum());
            children.add(up.get().getSecond());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private final LinkedList<Long> children;
        private Iterator<RecordId> currentIter;
        private final boolean isGreaterEqual;
        private DataBox key;
        ...
        public boolean hasNext() {
            ...
            while (!children.isEmpty()) {
                Long child = children.remove();
                BPlusNode childNode = BPlusNode.fromBytes(metadata, bufferManager, lockContext, child);
        ...
        private void addChildren(BPlusNode childNode) {
            if (childNode instanceof LeafNode) {
                currentIter = ((LeafNode) childNode).getRids().iterator();
                if (isGreaterEqual) {
                    int index = childNode.getFlatIndex(key);
                ...
                if (isGreaterEqual) {
                    int index = childNode.getFlatIndex(key);
                    List<Long> lst = ((InnerNode) childNode).getChildren();
                    children.addAll(0, lst.subList(index, lst.size()));
                } else {
                    children.addAll(0, ((InnerNode) childNode).getChildren());
                }
            }
        }
