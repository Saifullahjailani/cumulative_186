...
class InnerNode extends BPlusNode {
    ...
    @Override
    public int getFlatIndex(DataBox key) {
        int rangeIndex = Collections.binarySearch(keys, key);
        if(rangeIndex < 0) {
            rangeIndex = Math.abs(rangeIndex) - 1;
        }
        return rangeIndex;
    ...
    public LeafNode get(DataBox key) {
        ...
        return getChild(getIndex(key)).get(key);
    }
    ...
    private Optional<Pair<DataBox, Long>> split(){
        List<DataBox> rightKey = new ArrayList<>();
        List<Long> rightChilds = new ArrayList<>();
        int len = keys.size();
        int d = metadata.getOrder();
        int ptr = d+1; // to cover the last child
        for(int i = 0; i < ptr; i++) {
            rightKey.add(keys.remove(d)); // cut from d onward
            rightChilds.add(children.remove(d+1)); // cut from d+1 onward
        }
        DataBox newUpKey = rightKey.remove(0);
        InnerNode node = new InnerNode(metadata, bufferManager, rightKey, rightChilds, treeContext);
        node.sync();
        sync();
        return Optional.of(new Pair<>(newUpKey, node.getPage().getPageNum()));
    }
    // See BPlusNode.put.
    @Override
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        BPlusNode oneDown = getChild(index);
        Optional<Pair<DataBox, Long>> pair = oneDown.put(key, rid);
        if(!pair.isPresent()){
            return Optional.empty();
        }
        // the key and record Id that sent upward
        DataBox upKey = pair.get().getFirst();
        Long pageNum = pair.get().getSecond();

        keys.add(index, upKey);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        int sz = metadata.getOrder();
        while(data.hasNext()){
            if(splittable()){
                return split();
            }
            BPlusNode rightNode = getChild(keys.size());
            Optional<Pair<DataBox, Long>> upKey = rightNode.bulkLoad(data, fillFactor);
            if(upKey.isPresent()){
                int index = getIndex(upKey.get().getFirst());
                keys.add(index, upKey.get().getFirst());
                children.add(index+1, upKey.get().getSecond());
            }
        }
