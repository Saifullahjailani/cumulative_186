...
class LeafNode extends BPlusNode {
    ...
    private Optional<Pair<DataBox, Long>> split(){
        ...
        List<RecordId> rightRecs = new ArrayList<>();
        int len = keys.size();
        int d = metadata.getOrder();
        int ptr = d;
        while(ptr-- >= 0) {
            rightKey.add(keys.remove(d));
            rightRecs.add(rids.remove(d));
        }
        LeafNode rightNode = new LeafNode(metadata, bufferManager, rightKey, rightRecs, rightSibling,treeContext);
        rightNode.sync();
        rightSibling = Optional.of(rightNode.getPage().getPageNum());
        Pair<DataBox, Long> pair = new Pair<>(rightKey.get(0), rightSibling.get());
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        int fillSize = (int) Math.ceil(2 * metadata.getOrder() * fillFactor);
        while(data.hasNext()){
            Pair<DataBox, RecordId> pair = data.next();
            insert(pair.getFirst(), pair.getSecond());
            int size = keys.size();
            if(size >= fillSize + 1){
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // retrieved 1 bytes and advance pointer by one byte
        byte nodeType = buf.get();
        // check if the node isLeaf,
        assert (nodeType == (byte) 1);
        // this returns the page id of the right sibling, advances the pointer by 8 bytes
        // returns -1 if no right sibling exists
        ...
        for(int i = 0; i < numPairs; i++){
            ...
            RecordId id = new RecordId(pageNumber, entryNum);
            rids.add(id);

        }


        return new LeafNode(metadata, bufferManager, p, keys, rids, Optional.ofNullable(rightSibling == -1L ? null : rightSibling), treeContext);
    }
