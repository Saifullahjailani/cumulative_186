...
public class BNLJOperator extends JoinOperator {
    protected static final int SINGLE_BUFFER = 1;
    ...
    private class BNLJIterator implements Iterator<Record> {
        ...
        private void fetchNextLeftBlock() {
            ...
            // check if there is any page left in the left source
            if (!this.leftSourceIterator.hasNext()) {
                return;
            }
            // set the block iterator
            this.leftBlockIterator = getBlockIterator(this.leftSourceIterator, getLeftSource().getSchema(), numBuffers - 2);
            // set the reset point of the left block iterator so we can iterate over all right pages
            this.leftBlockIterator.markNext();
            // the left record iterates over the touples so it will get touples in current block
        ...
        private void fetchNextRightPage() {
            ...
            this.rightPageIterator = getBlockIterator(this.rightSourceIterator, getRightSource().getSchema(), SINGLE_BUFFER);
        ...
        private Record fetchNextRecord() {
            // TODO(proj3_part1): implement
            // solution: think as if you have double loops
            if (leftRecord == null) {
            ...
            while (true) {
                if (rightPageIterator.hasNext()) {
                    ...
                    // Case 2: The right page iterator doesn't have a value to
                    // yield but the left block iterator does
                    // get the next block
                    // reset the right page iterator so next block iterate over it
