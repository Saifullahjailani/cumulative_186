...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        // building
        // in the building part put building records into the hash table
        ...
        // probing
        // check if a match for the probing record exist in the hashtable or not
        for (Record probeRecord : probeRecords) {
            ...
            for (Record buildRecord : hashTable.get(probeJoinValue)) {
                this.joinedRecords.add(probeFirst ? probeRecord.concat(buildRecord) : buildRecord.concat(probeRecord));
            }
        }
    }
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        final int threshold = numBuffers - 2;
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            if (leftPartitions[i].getNumPages() <= threshold || rightPartitions[i].getNumPages() <= threshold) {
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        // it goes to next pass if the size of single partition is greater than 32
        for (int i = 0; i < 33; i++) {
