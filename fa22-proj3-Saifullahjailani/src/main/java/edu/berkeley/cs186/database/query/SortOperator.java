...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        int i = 0;
        for (Run run : runs){
            BacktrackingIterator<Record> iter = run.iterator();
            if(iter.hasNext()){
                priorityQueue.add(new Pair<>(iter.next(),i));
            }
        ...
        while (!priorityQueue.isEmpty()) {
            Pair<Record, Integer> RI = priorityQueue.poll();
            Record record = RI.getFirst();
            sortedRun.add(record);
            int index = RI.getSecond();
            if (iteratorList.get(index).hasNext()) {
                // add an iterator has more elemenets
                priorityQueue.add(new Pair<>(iteratorList.get(index).next(), index));
            }
        }
        return sortedRun;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (int i = 0; i < runs.size() - numBuffers + 2; i += numBuffers - 1) {
            List<Run> merged = runs.subList(i, Math.min(runs.size(), numBuffers + i - 1));
            merge.add(mergeSortedRuns(merged));
        }
        return merge;
    }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            ...
            merged.add(externalMerge);
        }

        // becuase buffer -1 in mergedPass
        for (int i = 0; i < merged.size() - 1; i++) {
            merged = mergePass(merged);
        }
