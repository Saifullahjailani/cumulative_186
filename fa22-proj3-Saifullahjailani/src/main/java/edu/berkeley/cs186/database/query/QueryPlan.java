...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (int colIndex : getEligibleIndexColumns(table)) {
            SelectPredicate predicate = selectPredicates.get(colIndex);
            QueryOperator altOp = new IndexScanOperator(this.transaction, table, predicate.column, predicate.operator, predicate.value);
            int altCost = altOp.estimateIOCost();
            if (minCost > altCost) {
                minOp = altOp;
                minCost = altCost;
                except = colIndex;
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        Set<String> set;
        QueryOperator joinPlan;
        for (Set<String> table : prevMap.keySet()) {
            for (JoinPredicate predicate : this.joinPredicates) {
                set = new HashSet<>();

                if (!table.contains(predicate.leftTable) && table.contains(predicate.rightTable)) {
                    ...
                    joinPlan = minCostJoinType(prevMap.get(table), pass1Map.get(set), predicate.rightColumn, predicate.leftColumn);
                } else if (table.contains(predicate.leftTable) && !table.contains(predicate.rightTable)) {
                    ...
                    joinPlan = minCostJoinType(prevMap.get(table), pass1Map.get(set), predicate.leftColumn, predicate.rightColumn);
                ...
                if (!result.containsKey(set)) {
                    result.put(set, joinPlan);
                }
                else if (result.get(set).estimateIOCost() > joinPlan.estimateIOCost()){
                        result.put(set, joinPlan);
                }
            }
        }
    ...
    public Iterator<Record> execute() {
        ...
        // initaialize both Maps
        for (String table : tableNames) {
            Set<String> tableMap = new HashSet<>();
            tableMap.add(table);
            QueryOperator plan = minCostSingleAccess(table);
            prevMap.put(tableMap, plan);
            pass1Map.put(tableMap, plan);
        }
        for (int i = 0; i < tableNames.size() - 1; i++) {
            pass1Map = minCostJoins(pass1Map, prevMap);
        }
        finalOperator = minCostOperator(pass1Map);
        addGroupBy();
        addProject();
        addSort();
        addLimit();
